/*
 * Модуль отвечающий за отправку и обработку запросов к серверу 
*/

var http = require("http");

var client = http.createClient(80, "ras.arbitr.ru");

var proxy;

/**
 * Метод получения списка решений
 * @param params - параметры
 * @param proxy - если нужно использовать то прокси {"host": 111, "port"}
 * @param callback - функция обратного вызова
 */
exports.getDecisions = function (_params, _proxy, callback) {
        
    var post_data = _params;
    proxy = _proxy;
    var data = "";
    ...
    if (proxy != null) {
        options = {
            host: proxy.host,
            port: proxy.port,
            path: 'http://ras.arbitr.ru/Ras/Search',
            method: "POST",
            headers: {
                'host': 'ras.arbitr.ru',
...
/**
 *Мето получения документа по его уникальному идентификатору
 *@pararms
 *  @param id - идентификатор документа
 *  @param load - обратный вызов в случае успеха
 *  @param error - вызов в случае ошибки
 */
exports.getDocument = function(params) {
    if (!params.id) {
        ...
        var options = {
            host: "172.20.99.56",
            port: 46522,
            path: "http://ras.arbitr.ru/HtmlDocument/" + params.id,
            headers: {
                Host: "ras.arbitr.ru"
            }};
...
/**
*  Метод получения списка актов и решений за заданый промежуток времени
*  @params - параметры 
*  @proxy - прокси
*  @callback - функция обратного вызова, в которую передастя список дел
 */
exports.start = function (proxy, callback) {
   ...
   this.getDecisions(params, proxy, function(data) {
       var docs = JSON.parse(data);

       docs.Result.Items.forEach(function(item){
           self.getDocument({
               id: item.Id,
               load: function(data) {
                   console.log(data);
